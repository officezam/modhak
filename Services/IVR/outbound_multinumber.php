...
{	
	...
	$contact_id	= $rowIvr['contact_id'];
	  ...
	  if($index == '' || $total_number == $index){ $last_index = '0'; }else{$last_index    = $index+1;}
	  ...
	  if($index == '')
	  {
		  ...
		  $dbo->insert_data('current_index' , $Insert_index);
			  ...
			  $update_index   = array('index_id'   =>  $last_index ) ;
			  $index_where    = 'WHERE compaign_id ='.$sv_id;
			  $dbo->update_data('current_index',$update_index,$index_where);
			  }
